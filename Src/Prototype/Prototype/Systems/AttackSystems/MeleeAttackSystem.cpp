...
void godot::MeleeAttackSystem::operator()(float delta, entt::registry& registry)
{
	...
	auto startAttackView = registry.view<entt::tag<CurrentWeaponMeleeTag>, MeleeAttackComponent, InputComponent>(entt::exclude<entt::tag<AttackedTag> >);
	startAttackView.less([this, &registry, currTimeMillis](entt::entity entity, MeleeAttackComponent& attackComp, InputComponent input)
	{
		...
		registry.assign<entt::tag<AttackedTag> >(entity);
	});

	lockSystem(delta, registry);
	animSystem(delta, registry);

	auto inputClearView = registry.view<entt::tag<AttackedTag> >();
	registry.remove<entt::tag<AttackedTag> >(inputClearView.begin(), inputClearView.end());
}