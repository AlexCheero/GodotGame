...
void godot::MeleeStateSystem::Tick(float delta, entt::registry& registry)
{
	auto view = registry.view<BotTag, MeleeAttackStateTag
		, MeleeAttackComponent, MeleePileInComponent, AttackCooldownComponent, HealthComponent, Spatial*
		, TargetLockComponent, DecisionMakingComponent>(entt::exclude<AttackPressedTag>);
	view.each([&registry](entt::entity entity, MeleeAttackComponent meleeComp, MeleePileInComponent pileInComp, AttackCooldownComponent cdComp, HealthComponent healthComp
		, Spatial* pSpatial, TargetLockComponent lockComp, DecisionMakingComponent decisionComp)
	{
		...
		bool attackInput = cdComp.prevHitTime + utils::SecondsToMillis(meleeComp.GetCurrentHit().attackTime) <= currTimeMillis;
		if (attackInput)
		...
		else if (registry.valid(lockComp.target) && pileInComp.maxPileInDistance < GetDistanceToTarget(registry, lockComp.target, pSpatial))
